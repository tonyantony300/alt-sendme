...
function DialogPopup({
	...
	/** Vertically center in the viewport instead of the default top-weighted placement. */
	centered?: boolean
}) {
	const stickToBottom = bottomStickOnMobile && !centered
			...
			>
				<DialogPrimitive.Popup
					className={cn(
						...
						stickToBottom &&
							'max-sm:max-w-none max-sm:rounded-none max-sm:border-x-0 max-sm:border-t max-sm:border-b-0 max-sm:opacity-[calc(1-min(var(--nested-dialogs),1))] max-sm:data-ending-style:translate-y-4 max-sm:data-starting-style:translate-y-4 max-sm:before:hidden max-sm:before:rounded-none max-sm:[--popup-safe-bottom:var(--safe-area-bottom)] max-sm:not-has-data-[slot=dialog-footer]:pb-(--safe-area-bottom)',
						className
					)}
	...
	return (
		<div
			className={cn(
				...
				variant === 'default' &&
					'border-t bg-muted/72 pt-4 pb-[calc(1rem+var(--popup-safe-bottom,0px))]',
				variant === 'bare' &&
					'in-[[data-slot=dialog-popup]:has([data-slot=dialog-panel])]:pt-3 pt-4 pb-[calc(1.5rem+var(--popup-safe-bottom,0px))]',
				className
			)}
