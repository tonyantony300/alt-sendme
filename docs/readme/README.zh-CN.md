...
2. 通过聊天、邮件或短信分享 ticket，**或**直接发送到已配对设备（桌面 / Android）。
3. 对方在应用中粘贴 ticket（或接受已配对设备的邀请），传输随即开始。

### 已配对设备

在 macOS、Windows、Linux 和 Android 上，您可以使用配对码在 **设置 → 设备** 中配对设备。配对之后：

- 分享时，发送方可在已配对设备旁点击 **Send**：无需手动复制 ticket。
- 当已配对的发送方发出邀请时，接收方会收到应用内提示（应用需处于打开状态）。
