...
- **Устройства поблизости в той же сети** — другие устройства DashBeam в вашей локальной сети появляются автоматически (mDNS). Сопрягайте в Настройках или отправляйте при обмене — без вставки ticket.
- **Фоновое присутствие** — на компьютере оставляйте приложение в трее или строке меню и при желании запускайте при входе в систему, чтобы сопряжённые устройства видели вас онлайн.
- **Системные уведомления** — запросы на сопряжение и приглашения к файлам могут показывать уведомления ОС, когда приложение не на переднем плане (настольные системы и Android).
...
2. Отправьте ticket через чат, email или SMS, **или** отправьте напрямую на сопряжённое или ближайшее устройство (настольный компьютер / Android).
3. Ваш друг вставляет ticket в приложение (или принимает приглашение), и передача начинается.

### Сопряжённые устройства

На macOS, Windows, Linux и Android вы можете сопрягать устройства в **Настройки → Устройства** с помощью кода сопряжения или приняв запрос на сопряжение от устройства поблизости в той же локальной сети. После сопряжения:

- Отправители могут нажать **Отправить** рядом с сопряжённым устройством при обмене: без ручного копирования ticket.
- Получатели получают запрос в приложении, когда сопряжённый отправитель приглашает их; при включённых системных уведомлениях они также могут получить баннер ОС, когда окно не в фокусе.
- На компьютере трей / строка меню может показывать, какие сопряжённые устройства онлайн, а DashBeam может продолжать работать после закрытия окна (**Настройки → Основные → Startup & background**).
- Ручные ticket и [sendme CLI](https://www.iroh.computer/sendme) работают точно так же, как и раньше.

### Устройства поблизости

Когда другие приложения DashBeam находятся в той же Wi-Fi или локальной сети, они могут появляться в разделе **Nearby** в **Настройки → Устройства** и в панели **Send to a device** при обмене:

- **Сопрягите** из Настроек, чтобы добавить устройство без обмена кодом сопряжения.
- **Отправьте** из панели обмена, чтобы пригласить устройство поблизости с текущим ticket; получатели подтверждают короткий код проверки перед принятием.
- Управляйте тем, могут ли другие вас находить, в **Настройки → Сеть → Your discoverability** (Everyone / Paired only / Off).

Nearby опирается на [mDNS](https://en.wikipedia.org/wiki/Multicast_DNS). Если сеть блокирует multicast (гостевой Wi-Fi, многие VPN), используйте ручной ticket или сопряжение через интернет — см. [Устранение неполадок](../troubleshooting.md#the-nearby-list-is-empty).
